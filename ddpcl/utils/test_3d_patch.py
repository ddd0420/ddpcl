...
def var_all_case_LA(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, data_path=''):
    # data_path = 'C:/PycharmProjects/dataset/LA'
    with open(data_path + '/test.list', 'r') as f:
        image_list = f.readlines()
    image_list = [data_path + "/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
...
def var_all_case_PAN(model, num_classes, patch_size=(96, 96, 96), stride_xy=16, stride_z=16, data_path=''):
    # la_path = 'C:/PycharmProjects/dataset/Pancreas'
    with open(data_path + '/test.list', 'r') as f:
        image_list = f.readlines()
    image_list = [data_path + "/Pancreas_h5/" + item.replace('\n', '') + ".h5" for item in image_list]
    loader = tqdm(image_list)
    total_dice = 0.0
    for image_path in loader:
        h5f = h5py.File(image_path, 'r')
...
def var_all_case_brats(model, num_classes, patch_size=(64, 128, 128), stride_xy=64, stride_z=64, data_path=''):
    # la_path = 'C:/PycharmProjects/dataset/BraTS2019'
    with open(data_path + '/val.txt', 'r') as f:
        image_list = f.readlines()
    image_list = [data_path + "/data/" + item.replace('\n', '') + ".h5" for item in image_list]
    loader = tqdm(image_list)
    total_dice = 0.0
    for image_path in loader:
        h5f = h5py.File(image_path, 'r')
